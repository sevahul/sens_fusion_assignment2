...
int main(int argc, char **argv)
{
	...
	BilterelarFiler(D_dp, output_D_bilet, gauss_ws, gauss_sigma_factor, sigmaRange);
	cv::imwrite((output_dir / "disp_Bilet.png").string(), output_D_bilet);
	if (gen_pointclouds)
		Disparity2PointCloud((output_dir / "pcl_Bilet").string(), output_D_bilet, dmin);
	if (show_images)
		cv::imshow("Bilterelar Filer D", output_D_bilet);

	JointBilterelarFiler(D_dp, I, output_D_JB, gauss_ws, gauss_sigma_factor, sigmaRange);
	cv::imwrite((output_dir / "disp_JB.png").string(), output_D_JB);
	if (gen_pointclouds)
		Disparity2PointCloud((output_dir / "pcl_JB").string(), output_D_JB, dmin);
	if (show_images)
		cv::imshow("Joint Bilterelar Filer D", output_D_JB);

	JointBileteralUpsamplingFilter(D_ds, I, output_D_JBU, gauss_ws, gauss_sigma_factor, sigmaRange);
	cv::imwrite((output_dir / "disp_JBU.png").string(), output_D_JBU);
	if (gen_pointclouds)
		Disparity2PointCloud((output_dir / "pcl_JBU").string(), output_D_JBU, dmin);
	if (show_images)
		cv::imshow("JBU D", output_D_JBU);

	IterativeUpsamplingFilter(D_ds, I, output_D_Iter, gauss_ws, gauss_sigma_factor, sigmaRange);
	cv::imwrite((output_dir / "disp_iter.png").string(), output_D_Iter);
	if (gen_pointclouds)
		Disparity2PointCloud((output_dir / "pcl_iter").string(), output_D_Iter, dmin);
	if (show_images)
